...
def dump_metainfo_and_dataset_to_txt(meta_info: DataMetaInformation, data_set, company_name):
    ...
    backup_path = RESULT_ROOT + '/' + str(
        meta_info.data_type.name) + '_' + meta_info.reporting_period + '_' + company_info.company_id
...
with dataland_datasets.ApiClient(configuration) as api_client:
    ...
    eu_taxo_meta_data_info = meta_data_api_instance.get_list_of_data_meta_info(show_only_active=True,
                                                                               data_type='eutaxonomy-financials')
