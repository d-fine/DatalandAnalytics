...
from dataland_datasets.models.company_associated_data_sfdr_data import (
    CompanyAssociatedDataSfdrData,
)
from dataland_datasets.models.company_associated_data_eu_taxonomy_data_for_financials import (
...
def dump_metainfo_and_dataset_to_txt(
    meta_info: DataMetaInformation, data_set, company_name
):
    path = (
        RESULT_ROOT
        + "/"
        + str(meta_info.data_type.name)
        + "_"
        + meta_info.reporting_period
        + "_"
        + company_name
    )
    backup_path = (
...
with dataland_datasets.ApiClient(configuration) as api_client:
    ...
    sfdr_meta_data_info = meta_data_api_instance.get_list_of_data_meta_info(
        show_only_active=True, data_type="sfdr"
    )
    eu_taxo_meta_data_info = meta_data_api_instance.get_list_of_data_meta_info(
        show_only_active=True, data_type="eutaxonomy-financials"
    )
    ...
    for item in all_meta_info:
        company_info = company_api_instance.get_company_by_id(
    ...
    eu_taxo_api_instance = dataland_datasets.EuTaxonomyDataForFinancialsControllerApi(
        api_client
    )
    ...
    for item in financials_meta_info:
        if item.data_type == DataTypeEnum.SFDR:
            ...
            dump_metainfo_and_dataset_to_txt(
                item, dataset, company_info.company_information.company_name
            )
        if item.data_type == DataTypeEnum.EUTAXONOMY_MINUS_FINANCIALS:
            dataset = eu_taxo_api_instance.get_company_associated_eu_taxonomy_data_for_financials(
                item.data_id
            )
            company_info = company_api_instance.get_company_by_id(
                company_id=item.company_id
