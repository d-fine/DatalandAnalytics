...
frameworks = ['vsme', 'sfdr', 'p2p', 'lksg', 'heimathafen', 'eutaxonomy-non-financials', 'esg-questionnaire',
              'eutaxonomy-financials']
reporting_periods = ['2020', '2021', '2022', '2023', '2024']
result = pd.DataFrame(columns=reporting_periods, index=frameworks.sort())
...
with dataland_datasets.ApiClient(configuration) as api_client:
    ...
    for (framework, year) in list(itertools.product(frameworks, reporting_periods)):
        list_of_meta_data_info = api_instance.get_list_of_data_meta_info(show_only_active=True, data_type=framework,
                                                                         reporting_period=year)
        result.loc[framework, year] = len(list_of_meta_data_info)
